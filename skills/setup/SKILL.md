...
kind: atomic
description: >
  Install, update, re-link, or verify the GDS AI Superpowers skill library on
  this machine. Use whenever the user says things like "set up
  gds-ai-superpowers", "install the skills", "update the skill library",
...
# Setup — manage the GDS AI Superpowers library
...
(`.claude/skills/threat-model -> <repo>/skills/threat-model`) rather than
...
This skill lives inside the repo at `skills/setup/`. The repo root is two
levels up (`setup/` → `skills/` → root). Resolve it before doing anything:

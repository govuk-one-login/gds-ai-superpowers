...
> Assisted draft. This verifies the **design commits** to the GDS Way
...
> One row per design-commitment control. State: current TD state → proposed commitment + cited `GDSW-*` → gap. For `GDSW-TEST-1`, record the **bar source**.

| `GDSW-*` | Control (commitment) | Current TD state | Proposed commitment + citation | Bar source (TEST-1 only) | Gap? |
|----------|----------------------|------------------|-------------------------------|--------------------------|------|
| GDSW-LANG-1 | Language chosen + supported | [present / absent] | [name the language; justify if new] | — | [y/n] |
| GDSW-TEST-1 | Per-`kind` coverage commitment | [present / absent] | [commit to a bar per tier] | [project .claude/standards \| GDS Way reference default] | [y/n] |
| GDSW-API-1 | Shared-contract change control | [present / absent] | [name the rule] | — | [y/n] |
| GDSW-API-2/4 | API versioning + back-compat strategy | [present / absent] | [the strategy] | — | [y/n] |
| GDSW-OPS-2/3 | Operability + docs approach | [present / absent] | [monitoring/alerting; ADRs] | — | [y/n] |
| GDSW-REVIEW-1 / SCM-1 | Review + source-control model | [present / absent] | [protected main; review before merge] | — | [y/n] |

## Security/accessibility cross-references (NOT ticked here)

> Where a GDSW control x-refs a security control, the engineering discipline is noted; the
...
|--------------|-------------------------------------|----------------------------|
| GDSW-OPS-1 (no PII in logs) | logs structured; no-PII rule in force | check-security-standards (A09 / MASVS-STORAGE-1) |
| GDSW-LANG-3 (deps) | lockfile + scanner exist | check-security-standards (A03:2025) |
| GDSW-SCM-2 (secrets) | secret scanning on; clean history | check-security-standards (A09) |
...
## Outcome at handoff
- **Commitments added to the TD:** [list of GDSW-*].

# GDSW-LANG-1 Supported language choice — deep

Pull when a language/stack choice is contested or "is this a new-language decision?" is unclear.

**Check**
- The tier's language is in the GDS-supported set already in use on the programme — for GDS,
  a deliberately small set: Node.js/TypeScript and Python 3 for backend/web; native Swift
  (iOS) and Kotlin (Android) for mobile. Go and Scala are not for new work.
...
- A team adding a new language for one service because it's familiar, with no TD decision —
  creating a long-tail support/skills burden (the Go/Scala lesson).
...
**Source**: `../languages.md` (lens) → GDS Way Programming languages (the supported set + exceptions).

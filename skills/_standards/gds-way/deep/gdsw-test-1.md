...
- Resolve the bar by precedence: the project's `.claude/standards/` value **wins**; else the
  gds-way reference default (backend 85 / web 90 / iOS 85 / Android 75). **Report the source**
  in the finding: `[source: project .claude/standards]` vs `[source: gds-way reference default]`.
...
- Treating a high % as quality — coverage guards quantity; pair with GDSW-TEST-3 (real assertions).

**Example / anti-example**
- *Bad:* "coverage is 80%, close enough" on a backend (bar is 85, gate not enforced).
- *Good:* gate fails at <85% backend; finding records `[source: gds-way reference default]`.

**Source**: `../testing.md` (lens). The numbers are this programme's, not an upstream GDS Way target.

# Engineering Way — Testing & coverage (`ENG-TEST-*`)
...
Scope note: this is functional test *discipline*. Performance/load testing is a later
concern; security/accessibility *testing* are governed by their own standards and cited
where they meet this surface.

## Controls

### `ENG-TEST-1` — a per-`kind` coverage bar exists and the gate enforces it
- **house convention:** this programme's coverage bars. The engineering way speaks of
  "appropriate test coverage" qualitatively. The *numbers* are therefore ours, owned here.
- **Checkable:** the repo defines a coverage bar for its tier (`kind`), and the CI gate
  ...
  **90%**, iOS **85%**, Android **75%**. Any skill applying the bar **reports its source**
  (`[source: project .claude/standards]` vs `[source: engineering-way reference default]`). An
...
### `ENG-TEST-2` — proportionate test shape, weighted to fast tests
- **realises:** test-driven development guidance (consider whether unit tests suffice or
...
### `ENG-TEST-3` — tests assert real behaviour, not tautologies
...
### `ENG-TEST-4` — the gate runs on every change and below-bar fails the build
- **realises:** continuous delivery and CI/CD standards.
...
- ENG-TEST-1 (coverage bar + source precedence) — deep: deep/eng-test-1.md
